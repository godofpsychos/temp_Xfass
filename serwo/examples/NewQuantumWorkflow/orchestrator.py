...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    krhl = yield context.call_activity("Invoke", serwoObject)
    akbd = yield context.call_activity("Transpile", krhl)
    hric = yield context.call_activity("Submit", akbd)
    xlps = yield context.call_activity("Poll", hric)
    ghsy = yield context.call_activity("Reconstruction", xlps)
    ghsy = insert_end_stats_in_metadata(ghsy)
    zanq = yield context.call_activity("CollectLogs", ghsy)
    return zanq
