...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    ymmn = yield context.call_activity("Source", serwoObject)
    trvn = yield context.call_activity("GenerateList", ymmn)
    wsff = yield context.call_activity("GenerateInteger", ymmn)
    fpjf = []
    fsvt = context.call_activity("Sine", wsff)
    jvna = context.call_activity("Cosine", wsff)
    bjll = context.call_activity("Factors", wsff)
    fpjf.append(fsvt)
    fpjf.append(jvna)
    fpjf.append(bjll)
    umzl = yield context.task_all(fpjf)
    nvpj = []
    ovzo = context.call_activity("GenerateMatrixA", ymmn)
    uire = context.call_activity("GenerateMatrixB", ymmn)
    nvpj.append(ovzo)
    nvpj.append(uire)
    eoxd = yield context.task_all(nvpj)
    yfjy = yield context.call_activity("Aggregator2", eoxd)
    ybcy = []
    gucw = context.call_activity("MatrixMultiplication", yfjy)
    tfeb = context.call_activity("LinPack", yfjy)
    ybcy.append(gucw)
    ybcy.append(tfeb)
    wwlg = yield context.task_all(ybcy)
    smta = []
    nwrq = context.call_activity("FastFourierTransform", trvn)
    bknp = context.call_activity("Aggregator1", umzl)
    gbvk = context.call_activity("Aggregator3", wwlg)
    smta.append(nwrq)
    smta.append(bknp)
    smta.append(gbvk)
    zorm = yield context.task_all(smta)
    elxk = yield context.call_activity("Aggregator4", zorm)
    elxk = insert_end_stats_in_metadata(elxk)
    zrha = yield context.call_activity("CollectLogs", elxk)
    return zrha
