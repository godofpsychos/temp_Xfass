...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    iigp = yield context.call_activity("Splitter", serwoObject)
    dmsn = yield context.call_activity("Transpiler0", iigp)
    lnek = yield context.call_activity("Transpiler1", iigp)
    bnfk = yield context.call_activity("Transpiler2", iigp)
    gxxo = []
    txua = context.call_activity("Simulator0", dmsn)
    zuyf = context.call_activity("Simulator1", lnek)
    iezk = context.call_activity("Simulator2", bnfk)
    gxxo.append(txua)
    gxxo.append(zuyf)
    gxxo.append(iezk)
    zqpi = yield context.task_all(gxxo)
    aoyi = yield context.call_activity("Reconstructor", zqpi)
    aoyi = insert_end_stats_in_metadata(aoyi)
    rqnw = yield context.call_activity("CollectLogs", aoyi)
    return rqnw
