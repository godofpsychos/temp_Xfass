...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    epla = yield context.call_activity("Splitter", serwoObject)
    boei = yield context.call_activity("Transpiler0", epla)
    ijiz = yield context.call_activity("Transpiler1", epla)
    opfl = []
    kwhb = context.call_activity("Simulator0", boei)
    gqwl = context.call_activity("Simulator1", ijiz)
    opfl.append(kwhb)
    opfl.append(gqwl)
    xmxs = yield context.task_all(opfl)
    rtsk = yield context.call_activity("Reconstructor", xmxs)
    rtsk = insert_end_stats_in_metadata(rtsk)
    zlps = yield context.call_activity("CollectLogs", rtsk)
    return zlps
