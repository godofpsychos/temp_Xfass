...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    ugdg = yield context.call_activity("Splitter", serwoObject)
    reoi = yield context.call_activity("Transpiler1", ugdg)
    hvih = yield context.call_activity("Transpiler0", ugdg)
    gwwu = []
    oyrc = context.call_activity("Simulator1", reoi)
    vvrl = context.call_activity("Simulator0", hvih)
    gwwu.append(oyrc)
    gwwu.append(vvrl)
    ryzz = yield context.task_all(gwwu)
    sydv = yield context.call_activity("Reconstructor", ryzz)
    sydv = insert_end_stats_in_metadata(sydv)
    oxcu = yield context.call_activity("CollectLogs", sydv)
    return oxcu
