...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    kmal = yield context.call_activity("TaskA", serwoObject)
    body=unmarshall(json.loads(kmal)).get_body()
    Time=body['time']
    jpra = kmal
    while True:
        deadline = context.current_utc_datetime + timedelta(seconds=Time)
        yield context.create_timer(deadline)
        jpra = yield context.call_activity("TaskB", jpra)
        body = unmarshall(json.loads(jpra)).get_body()
        if body["after_mod"] == 0:
            break
    jpra = insert_end_stats_in_metadata(jpra)
    qsnd = yield context.call_activity("CollectLogs", jpra)
    return qsnd
